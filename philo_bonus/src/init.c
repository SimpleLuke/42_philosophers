...
/*   Updated: 2024/02/11 17:00:59 by llai             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_sem(t_table *table)
{
	...
	if (table->print_sem == SEM_FAILED)
		err_exit(print_err("table->print_sem", "SEM print_sem error",
				EXIT_FAILURE), table);
	table->time_sem = sem_open("/time_sem", O_CREAT | O_EXCL, 0666, 1);
	if (table->time_sem == SEM_FAILED)
		err_exit(print_err("table->time_sem", "SEM time_sem error",
				EXIT_FAILURE), table);
}
